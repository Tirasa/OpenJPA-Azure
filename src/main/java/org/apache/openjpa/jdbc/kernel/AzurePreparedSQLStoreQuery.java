...
public class AzurePreparedSQLStoreQuery extends SQLStoreQuery {
    ...
    public AzurePreparedSQLStoreQuery(JDBCStore store) {
        super(store);
    }

    public StoreQuery.Executor newDataStoreExecutor(ClassMetaData meta, boolean subclasses) {
        return new AzurePreparedSQLStoreQuery.AzurePreparedSQLExecutor(this, meta);
    }
     ...
     *
     */
    public static class AzurePreparedSQLExecutor extends AbstractExpressionExecutor {

        private final ClassMetaData _meta;

        private final AzurePreparedSQLStoreQuery _query;

        public AzurePreparedSQLExecutor(AzurePreparedSQLStoreQuery q, ClassMetaData candidate) {
            _meta = candidate;
            _query = q;
        }
        ...
        public ResultObjectProvider executeQuery(StoreQuery q, Object[] params, StoreQuery.Range range) {
            ...
            try {
                ...
                if (getQueryExpressions()[0].projections.length > 0) {
                    ...
                    rop = new PreparedResultObjectProvider(
                            cachedSelect, (ClassMapping) _meta, store, fetch, res);
                } else {
                    rop = new SQLProjectionResultObjectProvider(
                            store, fetch, (ResultSetResult) res, q.getContext().getResultType());
                }
        ...
        /**
         * Convert given userParams to an array whose ordering matches as per expected during executeXXX() methods. The
         * given userParams is already re-parameterized, so this method have to merely copy the given Map values.
         *
