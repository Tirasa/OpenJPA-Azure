...
public class SQLAzurePreparedSQLStoreQuery extends PreparedSQLStoreQuery {

    private PreparedQueryImpl _cached;

    public SQLAzurePreparedSQLStoreQuery(JDBCStore store) {
        super(store);
    }

    public StoreQuery.Executor newDataStoreExecutor(ClassMetaData meta,
            boolean subclasses) {
        return new SQLAzurePreparedSQLStoreQuery.PreparedSQLExecutor(this, meta);
    }
    ...
    public static class PreparedSQLExecutor extends AbstractExpressionExecutor {
        ...
        private final SQLAzurePreparedSQLStoreQuery _query;

        public PreparedSQLExecutor(SQLAzurePreparedSQLStoreQuery q, ClassMetaData candidate) {
            _meta = candidate;
            _query = q;
        }
        ...
        public ResultObjectProvider executeQuery(StoreQuery q, Object[] params, StoreQuery.Range range) {
            ...
            SQLAzureRefCountConnection conn = (SQLAzureRefCountConnection) store.getConnection();
            ((SQLAzureDelegatingConnection) conn.getConn()).selectWorkingConnections();
