...
public class JDBCStoreQuery
    ...
    private static JDBCStoreQuery.ThreadLocalContext localContext = new JDBCStoreQuery.ThreadLocalContext();
    ...
    public void setContext(QueryContext ctx) {
        ...
        if (_store instanceof JDBCStoreManager && _store.getContext().getBroker().isClosed()) {
            ((JDBCStoreManager) _store).setContext(ctx.getStoreContext());
        }
    ...
    public Object newCompilationKey() {
        JDBCFetchConfiguration fetch = (JDBCFetchConfiguration) ctx.getFetchConfiguration();
    ...
    /**
     * Generate the selects with WHERE conditions needed to execute the query for the given mappings.
     */
    private boolean createWhereSelects(List sels, ClassMapping[] mappings,
        ...
        Number optHint = (Number) ctx.fetch.getHint(QueryHints.HINT_RESULT_COUNT);
        ClassMapping[] verts;
    ...
    /**
     * Return all the vertical mappings to select separately. Depends on subclass fetch mode and the type of query.
     */
    ...
    /**
     * The eager mode depends on the unique setting and range. If the range produces 0 results, use eager setting of
     * none. If it produces 1 result or the query is unique, use an eager setting of single. Otherwise use an eager mode
     * of multiple.
     */
    ...
    protected Number executeDelete(StoreQuery.Executor ex, ClassMetaData base,
    ...
    protected Number executeUpdate(StoreQuery.Executor ex, ClassMetaData base,
            ClassMetaData[] metas, boolean subclasses, ExpressionFactory[] facts,
            QueryExpressions[] exps, Object[] params) {
        return executeBulkOperation(metas, subclasses, facts, exps,
    ...
    private Number executeBulkOperation(ClassMetaData[] metas,
        ...
        for (int i = 0; i < mappings.length; i++) {
            ...
            if (!isUpdate) {
                ...
                LifecycleEventManager mgr = ctx.getStoreContext().getBroker().getLifecycleEventManager();
    ...
    /**
     * Return the single table for the given fields, or INVALID if they use multiple tables.
     */
    ...
    /**
     * Return the table for the field if the given table hasn't been set yet, or if the tables match. If the field uses
     * a different table, returns INVALID. Also returns INVALID if field is dependent.
     */
    ...
    protected String[] getDataStoreActions(ClassMetaData base,
            ClassMetaData[] metas, boolean subclasses, ExpressionFactory[] facts,
            QueryExpressions[] exps, Object[] params, StoreQuery.Range range) {
        Context[] ctxs = new Context[exps.length];
        for (int i = 0; i < exps.length; i++) {
            ctxs[i] = exps[i].ctx();
        }
    ...
    protected int executeUpdate(Connection conn, PreparedStatement stmnt,
            SQLBuffer sqlBuf, boolean isUpdate)
            throws SQLException {
    ...
    private Object handleTrimVal(Object value, Object ob, Object[] params,
        ...
        char trimCharObj = ((String) getValue(trimChar, ob, params, sm)).charAt(0);
